...
void    set_pixel(t_fdf *m_fdf, t_point *point)
{
    ...
    {
        byte = (point->x * 4) + (point->y * m_fdf->size_line);
...
void    brez_draw(t_fdf *m_fdf, t_brez *brez)
{
    ...
    while (brez->point1.x != brez->point2.x || brez->point1.y != brez->point2.y)
    {
        set_pixel(m_fdf, &brez->point1);
        tmp_err = brez->err;
        if (tmp_err > -brez->d_x && brez->point1.x != brez->point2.x)
        {
            brez->err -= brez->d_y;
            brez->point1.x += brez->sign_x;
        }
        if (tmp_err < brez->d_y && brez->point1.y != brez->point2.y)
        {
            brez->err += brez->d_x;
